...
function NewTask(){
    const {handleCreate, newTaskState, setNewTaskState, setMessageTitleError, messageTitleError} = useTasks();
                ...
                {({handleChange, handleSubmit, values, isSubmitting}) => (
                    <Form className="w-full min-h-[400px] h-auto p-10 flex flex-col justify-center items-center" onSubmit={handleSubmit}>
                        <FontAwesomeIcon icon={faArrowLeft} onClick={() => {
                            setNewTaskState(prevState => !prevState) 
                            setMessageTitleError(0)}
                        } 
                            className="absolute z-20 top-4 right-4 text-3xl font-semibold text-[#a0f] hover:scale-110 cursor-pointer" />
                        ...
                        <h2 className="text-transparent bg-clip-text text-center bg-gradient-to-r from-[#00f] to-[#a0f] font-bold text-lg mb-4">CREATE NEW TASK</h2>
                            <div className="w-full h-full flex flex-col mb-5">
                                ...
                                <input name="title" className={messageTitleError ? "animate-wiggle border-red-500 rounded-md bg-white focus:placeholder:text-white text-[#a0f] focus:text-white focus:bg-gradient-to-tr from-[#00f] to-[#a0f] outline-none p-2 border" : "rounded-md bg-white focus:placeholder:text-white text-[#a0f] focus:text-white focus:bg-gradient-to-tr from-[#00f] to-[#a0f] outline-none p-2 border border-[#a0f]"} type="text" placeholder="Write a title" onChange={handleChange} value={values.title}/>
                                {messageTitleError !== 0 ? <span className="text-red-600 text-sm translate-x-1">{messageTitleError === 2 ? "This task must have title." : "the title must have less than 20 characters"}</span> : <span></span>}
                            </div>
